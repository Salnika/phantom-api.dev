...
import { apiService as tokenApiService } from '@/services/api';
...
const ApiTokens: React.FC = () => {
  ...
  const handleGenerateToken = async () => {
    try {
      setLoading(true);
      setError('');
      const response = await apiService.generateToken({ role, expiresIn });
      if (response.success) {
        setGeneratedToken(response.data.token);
        setTokenInfo({
          role: response.data.payload.role,
          expiresIn: response.data.expiresIn,
          resourceAccess: response.data.payload.resourceAccess
        });
        // Refresh token list after generating a new token
        loadTokens();
        loadStats();
      } else {
  ...
  const loadTokens = async () => {
    try {
      ...
      const response = await tokenApiService.getTokens(params);
  ...
  const loadStats = async () => {
    try {
      const response = await tokenApiService.getTokenStats();
      if (response.success && response.data) {
  ...
  const handleCreateToken = async () => {
    try {
      ...
      const response = await tokenApiService.createToken(formData);
      if (response.success) {
  ...
  const handleRevokeToken = async (tokenId: string) => {
    ...
    try {
      const response = await tokenApiService.revokeToken(tokenId);
      if (response.success) {
        loadTokens();
        loadStats();
      }
  ...
  const handleCleanupExpired = async () => {
    ...
    try {
      const response = await tokenApiService.cleanupExpiredTokens();
      if (response.success) {
        loadTokens();
        loadStats();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Generate new tokens and manage existing JWT tokens
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle>Create Advanced Token</DialogTitle>
              </DialogHeader>
      ...
      <Card className="card-hover">
        <CardHeader>
          <CardTitle>Quick Token Generation</CardTitle>
          <CardDescription>Generate a standard API token quickly</CardDescription>
        </CardHeader>
        <CardContent className="content-spacing">
          <div className="alert alert-info mb-4">
            <h4 className="font-semibold text-foreground mb-2">Role Permissions:</h4>
            <ul className="list-disc pl-5 compact-spacing text-foreground">
              <li><strong>Anonymous:</strong> Read-only access to all resources</li>
              <li><strong>User:</strong> Create, read, and update own resources</li>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="form-group">
              ...
              <Select value={role} onValueChange={setRole}>
                ...
                <SelectContent>
                  <SelectItem value="anon">Anonymous (Read only)</SelectItem>
          ...
          {generatedToken && (
            <div className="alert alert-success animate-scaleIn mt-4">
              <h4 className="mb-2 flex items-center gap-2 font-semibold">
                <CheckCircle size={20} />
                Token Generated Successfully!
              </h4>
              {tokenInfo && (
                <div className="mb-2 text-sm item-spacing">
                  <p><strong>Role:</strong> {tokenInfo.role}</p>
                  <p><strong>Expires:</strong> {tokenInfo.expiresIn}</p>
                  <p><strong>Access:</strong> {tokenInfo.resourceAccess}</p>
                </div>
              )}
              <div className="bg-muted p-3 rounded-md font-mono text-sm border mb-2 break-all">
                {generatedToken}
              </div>
              <Button onClick={handleCopyToken} variant={copied ? 'secondary' : 'outline'} className="gap-2">
                <Copy size={16} />
      ...
      <Card>
        <CardHeader>
          <CardTitle>Existing Tokens ({filteredTokens.length})</CardTitle>
          <CardDescription>
      ...
      <Card className="card-hover">
        ...
        <CardContent className="content-spacing">
          <div className="responsive-text leading-relaxed content-spacing">
            <h4 className="text-base font-semibold text-foreground">Using with curl:</h4>
            <pre className="bg-muted p-3 rounded-md overflow-auto scrollbar-thin">
              {`curl -X POST http://localhost:3000/api/User/create \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer YOUR_TOKEN_HERE" \\
  -d '{"email": "user@example.com", "name": "John Doe"}'`}
            </pre>
            <h4 className="text-base font-semibold text-foreground">Using with phantom-api client:</h4>
            <pre className="bg-muted p-3 rounded-md overflow-auto scrollbar-thin">
